...
def get_height(noise_heightmap, x, y, can_down=False):
    ...
    if can_down and (h <= -0.7 or (h <= -0.42 and randint(0, 100 - abs(int(h*100))) <= 22)):
        return max(-60, int(70*h**3))
...
def give_size(arr: list, image):
    ...
    #Морской царь
    quests = [cur for cur in arr if (450 <= (cur[0] ** 2 + cur[2] ** 2) ** 0.5 <= 700) and is_normal_height(cur[1])]
    for x in quests: x[-1]+=200
    #Песчаная библиотека
    quests = [cur for cur in arr if (800 <= (cur[0] ** 2 + cur[2] ** 2) ** 0.5 <= 1200) and is_normal_height(cur[1])]
    for x in quests: x[-1]+=300
    #Элеум Лойс
    quests = [cur for cur in arr if (1400 <= (cur[0] ** 2 + cur[2] ** 2) ** 0.5 <= 1800) and is_normal_height(cur[1])]
